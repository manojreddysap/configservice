...
import json
import os
from main_copy import force_worker_update, monitor_worker_update, access_token_config, access_token_trm
...
worker_update = os.getenv("ForceWorkerUpdate")
try:
...
config_url = json.dumps(config[system_id]['config_url']).strip('\"')
...
def set_tenant_specific_worker_config():
    url = f"{config_url}/configurations?scope={scope}&scopeId={tenant_name}"
    ...
    headers = {
        'Authorization': f'Bearer {access_token_config}',
...
def get_global_worker_config():
    url = f"{config_url}/configurations?scope={scope}"
...
if scope == "TENANT":
    ...
    if action == "READ":
        ...
        if set_tenant_specific_worker_config() == 200:
            ...
            print(f"{set_tenant_specific_worker_config()} Configuration setup was not possible. please check if you are passing correct parameters."
              "if any doubts please send a mail to infra")
    ...
    if worker_update == "true":
        ...
        print("Force Worker update has not been opted for, hence not updating the worker")
